...
function BuildDevelopReleaseImage {
    ...
    Set-Location $currentDir\$folder_name\$folder_name\

    # Build develop image
    Write-Host "`n[$folder_name] Building develop image ..."
    $output = & .\build.develop.ps1 *>&1
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        Write-Host $output # Show the output if it fails
        exit 1
    }

    # Build release image
    Write-Host "[$folder_name] Building release image ..."
    $output = & .\build.release.ps1 *>&1
